...
export function Cart({ onContinueShopping, onProceedToDelivery }: Props) {
    ...
    // const [isCheckingOut, setIsCheckingOut] = useState(false); // Removed unused state

    const handleCheckout = async () => {
        const isDemoMode = import.meta.env.VITE_DEV_MODE === 'true';
        if (isDemoMode) {
            alert('デモモード: 注文完了（実際には送信されません）');
            return;
        }
...
`);
        ...
        // Quote Required Flow checks...

        // Standard Flow: Proceed to Delivery Entry (instead of direct API checkout)
        // User requested: "Go to place where you can select delivery location" and "Don't make final payment part"
        onProceedToDelivery();
        // setIsCheckingOut(true); // Logic stops here for this revision
    };
    /*
        setIsCheckingOut(true);
        try {
            // ... (rest of old logic hidden/removed as requested)
        }
    */
    /*
    try {
        // Calculate surcharge again to be safe/consistent pass
        const surcharge = cartDeliveryMode === 'rush'
            ? Math.floor(subtotal * deliverySettings.rushSurchargeRate)
            : 0;
        ...
        console.error(error);
        alert('決済ページへの移動に失敗しました。管理者にお問い合わせください。\n' + error);
