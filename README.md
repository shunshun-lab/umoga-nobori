...
### のぼり見積もり機能
- **リアルタイム見積もり**: サイズ・生地・オプション・枚数を選択すると即座に合計金額を算出。
- **数量割引 (Volume Discount)**: 枚数に応じた割引率（10%〜25%OFF）を自動適用。Google Sheetでルール変更可能。
- **カスタムサイズ対応**: 定型サイズ以外の寸法も入力可能 (10cm単位計算)。
- **オプション選択**: 防炎加工、棒袋、防水などのオプション加算に対応。

### 注文・決済機能
- **Shopify カート連携**: 見積もり内容を商品としてカートに追加。
- **ダイナミック注文作成**: Admin APIを使用し、カスタム仕様を含む注文データを動的に生成。
- **配送日時シミュレーション**: 通常便・お急ぎ便の選択と出荷予定日の表示。

### 管理・運用機能
- **Google Sheets 同期**: 割引率などのマスタデータをスプレッドシートから取り込み。
- **デザインデータ入稿**: カート追加時にファイルアップロード（ドラッグ＆ドロップ）に対応。
